...
<div class="container mt-4 mb-4">
	<div class="row view-offers">
		<h2 class="mb-3 ml-3"><?= $lang['view_offers']; ?> (<?= $count_offers; ?>) </h2>
		<div class="col-md-12">
			<div class="card mb-4 rounded-0">
				<div class="card-body">
					<h5 class="font-weight-bold"> <?= $lang['request_desc'];?>: </h5>
					<p class="offer-p"><?= $request_description; ?></p>
					<p class="offer-p">
					<i class="fa fa-money"></i> <span> <?= $lang['request'];?>s<?php $budget_kl = mb_strtolower($lang['th']['budget']);echo $budget_kl;?> : </span><span class="text-muted"> <?= showPrice($request_budget); ?> </span><br>
					<i class="fa fa-calendar"></i> <span> <?= $lang['request'];?>s<?php $date_kl = mb_strtolower($lang['th']['date']);echo $date_kl;?></span><span class="text-muted"> <?= $request_date; ?></span> <br>
					<i class="fa fa-clock-o"></i> <span><?= $lang['request'];?>s<?php $duration_kl = mb_strtolower($lang['order_details']['duration']);echo $duration_kl;?>: </span><span class="text-muted">  <?= $request_delivery_time; ?> </span>  <br>
					<i class="fa fa-archive"></i> <span> <?= $lang['request'];?>s<?php $category_kl = mb_strtolower($lang['label']['category']);echo $category_kl;?>: </span><span class="text-muted"> <?= $request_cat_title; ?> / <?= $request_child_title; ?> </span>
					</p>
				</div>
			</div>
			...
			<div class="card rounded-0 mb-3">
				<div class="card-body">
					<h3 class="text-center"> 
						<i class="fa fa-frown-o"></i> <?= $lang['no_offer_now'] ?>
					</h3>
				</div>
			</div>
			<?php }else{ ?>
