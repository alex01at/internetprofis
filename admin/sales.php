    ...
    <div class="col-lg-12">
      ...
      <div class="card">
        ...
        <div class="card-header">
          <!--- card-header Starts --->
          <ol class="text-right">
            <a href="index?expenses" class="btn btn-success">
              <i class="text-white"></i> <span class="text-white">Expenses</span>
            </a>
          </ol>
          ...
          <div class="offset-lg-3 col-md-6">
            <form action="" method="get">
              ...
              <div class="input-group mb-3 mt-3 mt-lg-0">
                <input type="date" name="from" value="<?= (isset($_GET['from']))?$input->get('from'):''; ?>"
                  max="<?= date("Y-m-d") ?>" class="form-control" placeholder="From">
                <input type="date" name="to" value="<?= (isset($_GET['to']))?$input->get('to'):''; ?>"
                  max="<?= date("Y-m-d") ?>" class="form-control" placeholder="To">
          ...
          <div class="table-responsive">
            <table class="table table-bordered">
              ...
              <tbody>
                ...
                <?php
  $per_page = 10;
  $page = 1; // Standardseite, falls keine 'sales'-Parameter im GET-Array vorhanden ist
  if (isset($_GET['sales'])) {
      // Überprüfen, ob der 'sales'-Parameter im GET-Array vorhanden ist
      $page = intval($_GET['sales']); // Wandeln Sie die Zeichenkette in eine Ganzzahl um
      if ($page <= 0) {
          // Stellen Sie sicher, dass die Seite nicht kleiner oder gleich 0 ist
          $page = 1;
      }
  }
  $i = ($page - 1) * $per_page; // Berechnen Sie den Wert von $i basierend auf $page und $per_page
  	...
  	?>
                  ...
                  <td>
                    ...
                    <a class="btn btn-primary text-white" target="_blank"
                      href="index?single_order=<?=$sale->work_id; ?>">
                      <i class="fa fa-info-circle"></i> View Order
                    </a>
                    <?php }else{ ?>
                    <a class="btn btn-primary text-white" target="_blank"
                      href="../proposals/<?=$seller_user_name; ?>/<?=$proposal->proposal_url; ?>">
