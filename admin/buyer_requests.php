        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <a href="index?buyer_requests&status=active"
                        class="<?php if($status == " all "){ echo "text-muted "; } ?> mr-2">
                        Active (<?= $count_active_requests; ?>)
                    </a>
                    <span class="mr-2">|</span>
                    <a href="index?buyer_requests&status=pending"
                        class="<?php if($status == " all "){ echo "text-muted "; } ?> mr-2">
                        Pending (<?= $count_pending_requests; ?>)
                    </a>
                    <span class="mr-2">|</span>
                    <a href="index?buyer_requests&status=unapproved"
                        class="<?php if($status == " all "){ echo "text-muted "; } ?> mr-2">
                        Declined (<?= $count_unapproved_requests; ?>)
                    </a>
                    <span class="mr-2">|</span>
                    <a href="index?buyer_requests&status=pause"
                        class="<?php if($status == " all "){ echo "text-muted "; } ?> mr-2">
...
if (isset($_GET['buyer_requests'])) {
    $page = intval($_GET['buyer_requests']); // Stellen Sie sicher, dass $page ein Integer ist
    if ($page == 0) {
        $page = 1;
    }
} else {
                           ...
                           ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="index?approve_request=<?= $request_id; ?>" class="btn btn-success"
                                            onclick="return confirm('You are about to approve this request. Continue?')">
                                            <i class="fa fa-thumbs-up text-white" style="width:12px;"></i> <span
                                                class="text-white pl-1">Approve</span>
                                        </a>
                                        <div class="mb-2"></div>
                                        <a href="index?unapprove_request=<?= $request_id; ?>" class="btn  pt-1"
                                            onclick="return confirm('You are about to delete this request perminently. Continue?')"
                                            style="background-color:red;">
                                            <i class="fa fa-thumbs-down text-white" style="width:23px;"></i> <span
                                                class="text-white">Decline</span>
                                        </a>
                                        <div class="mb-2"></div>
                                        <?php } ?>
                                        <a href="index?delete_request=<?= $request_id; ?>" class="btn btn-danger pt-1"
                                            onclick="return confirm('You are about to delete this request perminently. Continue?')">
                                            <i class="fa fa-trash text-white" style="width:23px;"></i> <span
