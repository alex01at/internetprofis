...
if(isset($_POST['update_bar'])){
  ...
  if($update_bar){
    echo "
    <script>alert_success('Announcement Bar Settings Has Been Updated Successfully.','index?theme_settings')</script>";
  }
...
<div class="card-header">
  <!--- card-header Starts ---> <ol class="text-right">
        <li class="active">
        <button type="button" class="btn btn-success" data-toggle="modal" data-target="#addSlide">
  Add new Slide
</button>
        </li>
      </ol>
  ...
  <div class="modal-dialog mw-100 w-75">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Add new Slider for loggedin Userdashboard</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal Body -->
      <div class="modal-body">
        <?php include('insert/insert_slide.php'); ?>
      </div>
...
<li class="nav-item">
<a class="nav-link" data-toggle="pill" href="#legal">Legal Column</a>
</li>
...
<div id="legal" class="tab-pane fade in"><!--- categories tab-pane fade show active Starts --->
<?php
$get_footer_links = $db->select("footer_links",array("link_section" => "legal","language_id" => $adminLanguage));
while($row_footer_links = $get_footer_links->fetch()){
$link_id = $row_footer_links->link_id;
$link_title = $row_footer_links->link_title;
$link_section = $row_footer_links->link_section;
...
<input type="text" name="link_title" placeholder="Link Title" class="form-control mb-3 mr-sm-2 mb-sm-0" required="">
<input type="text" name="link_url" placeholder="Link Url" class="form-control mb-3 mr-sm-2 mb-sm-0" required="">
<button type="submit" class="btn btn-success form-control" name="add_legal">
<i class="fa fa-plus-circle"></i> Add Link
</button>
</form>
</div><!--- bg-light p-3 Ends --->
...
$link_title = $input->post("link_title");
$link_url = $input->post("link_url");
$insert_link = $db->insert("footer_links",array("language_id"=>$adminLanguage,"link_title"=>$link_title,"link_url"=>$link_url,"link_section"=>'legal'));
if($insert_link){
echo "<script>window.open('index?theme_settings#footer','_self');</script>";
}	
}
?>
</div><!--- legal tab-pane fade show active Ends --->
...
$enable_copyright = $row_general_settings->enable_copyright;
...
<div class="card-header"><!--- card-header Starts --->
<h4 class="h4">
<i class="fa fa-home fa-fw"></i>  Footer End Copyright Text
<small class="text-muted">
This will display everywhere at the bottom of your site
</small>
</h4>
</div><!--- card-header Ends --->
<div class="card-body"><!--- card-body Starts --->
...
<label class="col-md-3 control-label"> Enable Copyright : </label>
<div class="col-md-6">
<select name="enable_copyright" class="form-control" required="">
    <option value="1" <?php if($enable_copyright == 1){ echo "selected"; } ?>> Yes </option>
    <option value="0" <?php if($enable_copyright == 0){ echo "selected"; } ?>> No </option>
  </select>
</div></div>
<div class="form-group row"><!--- form-group row Starts --->
...
<label class="col-md-3 control-label"></label>
<div class="col-md-6">
<input type="submit" name="update_copyright" class="form-control btn btn-success" value="Update Copyright">
</div>
</div><!--- form-group row Ends --->
</form><!--- form Ends --->
</div><!--- card-body Ends --->
...
if(isset($_POST['update_copyright'])){
  ...
  $enable_copyright = $input->post('enable_copyright');
  $update_copyright = $db->update("general_settings",array(
    "site_copyright" => $site_copyright,
    "enable_copyright"=> $enable_copyright
  ));
  if($update_copyright){
    $insert_log = $db->insert_log($admin_id,"update_copyright","","updated");
    
      echo "<script>alert_success('Copyright Section has been set successfully.','index?theme_settings');</script>";
