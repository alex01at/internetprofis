			...
			<div class="p-3 mb-3">
				...
				<form class="form-inline pb-2" method="get" action="">
					...
					<div class="form-group">
						<label> Date : </label>
						<input type="date" name="date" value="<?= @$input->get('date'); ?>"
							class="form-control mb-2 mr-sm-2 ml-1 mb-sm-0">
					</div>
					<div class="form-group">
						<label> Admin : </label>
	...
	<div class="row">
		...
		<div class="col-lg-12">
			...
			<div class="card">
				...
				<div class="card-body">
					<!--- card-body Starts --->
					<a href="index?delete_all_logs"
						onclick="return confirm('Do you really want to delete all admin panel logs.')"
						class="btn btn-danger btn-lg float-right mb-4"><i class="fa fa-trash"></i>&nbsp;&nbsp;Delete All
						Logs</a>
...
if(isset($_GET['admin_logs'])){
    $page = intval($_GET['admin_logs']); // Konvertiert den Wert zu einer ganzen Zahl
    if($page <= 0) {
        $page = 1; // Seitennummer darf nicht kleiner oder gleich 0 sein
    }
} else {
    $page = 1;
}
...
$i = $start_from;
// Verwendung von $i in einer Schleife, um Daten anzuzeigen
// for($i = $start_from; $i < $start_from + $per_page; $i++) {
//     // Zeige Datensatz mit Index $i an
// }
// print_r($input->get());
...
}else if ($work == "payment") {
$work = "<a href='index?payment' target='_blank'>Payment Settings</a>";
								...
								<tr>
									...
									<td>
										<a href="index?delete_log=<?= $id; ?>" class="btn btn-danger text-white">
											<i class="fa fa-trash text-white"></i> <span
