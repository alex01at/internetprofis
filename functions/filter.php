...
function get_proposals($filter_type){
   ...
   global $input, $siteLanguage, $db, $enable_referrals, $lang, $dir, $s_currency, $login_seller_id, $videoPlugin, $site_url;
   ...
   switch($filter_type){
      case "search":
         ...
         $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_title LIKE :proposal_title AND proposal_status='active'", $values);
         break;
      case "category":
         if(isset($_SESSION['cat_id'])){
            $session_cat_id = $_SESSION['cat_id'];
            $values["cat_id"] = $session_cat_id;
            $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_cat_id=:cat_id AND proposal_status='active'", $values);
         } elseif(isset($_SESSION['cat_child_id'])){
            $session_cat_child_id = $_SESSION['cat_child_id'];
            $values["child_id"] = $session_cat_child_id;
            $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_child_id=:child_id AND proposal_status='active'", $values);
         }
         break;
      case "featured":
      ...
      case "tag":
         if(isset($_SESSION['tag'])){
            $tag = $_SESSION['tag'];
            $values["tag"] = "%$tag%";
            $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_tags LIKE :tag AND proposal_status='active'", $values);
         }
         break;
      default:
         return;
   ...
   // Online-Verkäufer ermitteln
   if(isset($get_proposals) && $get_proposals){
      while($row_proposals = $get_proposals->fetch()){
         $proposal_seller_id = $row_proposals->proposal_seller_id;
         $select_seller = $db->select("sellers", ["seller_id" => $proposal_seller_id]);
         $sellerData = $select_seller->fetch();
         if($sellerData && $sellerData->seller_status && check_status($proposal_seller_id) === "Online"){
            $online_sellers[] = $proposal_seller_id;
         }
      }
   }
...
function get_pagination($filter_type){
  ...
  while($row_proposals = $get_proposals->fetch()){
    $proposal_seller_id = $row_proposals->proposal_seller_id;
    $select_seller = $db->select("sellers", ["seller_id" => $proposal_seller_id]);
    $seller = $select_seller->fetch();

    if($seller && isset($seller->seller_status)){
        $seller_status = $seller->seller_status;
        if(check_status($proposal_seller_id) == "Online"){
            array_push($online_sellers, $proposal_seller_id);
        }
    } else {
        // Optional: Loggen, dass der Verkäufer nicht gefunden wurde
        // error_log("Seller with ID $proposal_seller_id not found.");
    }
}
