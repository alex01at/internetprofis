  ...
  <div class="col-lg-8 col-md-7 mb-3"><!--- col-lg-8 col-md-7 mb-3 Starts --->
    <div class="card rounded-0 mb-5 border-0">
      <div class="card-body details pt-0">
        ...
        ?>
        <span class="text-muted span"> (<?= $count_reviews; ?>) &nbsp;<?= $proposal_order_queue; ?> <?= $lang['order_queues']; ?>.</span>
    ...
    <div class="card-header"><h5><?= $lang["compare_packages"];?></h5></div>
    ...
    <tr class="<?=($lang_dir == "right" ? 'text-right':'')?>" width="20">
      <td class="b-ccc"><?=$lang['label']['description'];?></td>
    ...
    <tr>
      <td class="b-ccc" width="150"> <?=$lang['order_details']['revisions'];?> </td>
      <td><?= ucwords($p_revisions_1); ?></td>
      <td><?= ucwords($p_revisions_2); ?></td>
      <td><?= ucwords($p_revisions_3); ?></td>
    </tr>
    <tr>
      <td class="b-ccc" width="150"><?= $lang["sidebar"]["delivery_time"];?> </td>
      <td><?= $p_delivery_time_1; ?> <?=$lang["day"];?></td>
      <td><?= $p_delivery_time_2; ?> <?=$lang["day"];?></td>
      <td><?= $p_delivery_time_3; ?> <?=$lang["day"];?></td>
    </tr>
    <tr>
      <td class="b-ccc"> <?=$lang['order_details']['quantity'];?> </td>
      <td>
        <form method="post" action="../../checkout">
          ...
          <a class="btn btn-success text-white btn-block" href="../edit_proposal?proposal_id=<?= $proposal_id; ?>&pricing"><?=$lang["titles"]["edit_proposal"];?></a>
          <?php }else{ ?>
          <button class="btn btn-success text-white btn-block" type="submit" name="add_order" <?= $disabled; ?>><?= $lang["select"]; ?> <?= showPrice($p_price_1); ?></button>
          <?php } ?>
        </form>
      </td>
      <td>
        <form method="post" action="../../checkout">
          ...
          <a class="btn btn-success text-white btn-block" href="../edit_proposal?proposal_id=<?= $proposal_id; ?>&pricing"><?=$lang["titles"]["edit_proposal"];?></a>
          <?php }else{ ?>
          <button class="btn btn-success text-white btn-block" type="submit" name="add_order" <?= $disabled; ?>><?= $lang["select"]; ?> <?= showPrice($p_price_2); ?></button>
          <?php } ?>
        </form>
      </td>
      <td>
        <form method="post" action="../../checkout">
          ...
          <a class="btn btn-success text-white btn-block" href="../edit_proposal?proposal_id=<?= $proposal_id; ?>&pricing"><?=$lang["titles"]["edit_proposal"];?></a>
          <?php }else{ ?>
          <button class="btn btn-success text-white btn-block" type="submit" name="add_order" <?= $disabled; ?>><?= $lang["select"]; ?> <?= showPrice($p_price_3); ?></button>
          <?php } ?>
        </form>
      </td>
    </tr>
    ...
    <div class="card rounded-0 mb-5 <?=($lang_dir == "right" ? 'text-right':'')?>" id="details">
      <div class="card-header"><h4><?=$lang['proposal']['about'];?></h4></div>
    ...
    <div class="card proposal-reviews rounded-0 mb-5" id="reviews">
      <div class="card-header">
        ...
        <span class="mr-2"> <?= $count_reviews; ?> <?= $lang['proposal']['nav']['reviews']; ?> </span>
        <?php
        for($proposal_i=0; $proposal_i<$proposal_rating; $proposal_i++){
