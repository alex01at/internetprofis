		...
		if(isset($_SESSION['seller_user_name'])){
			if($login_seller_id != $proposal_seller_id){
				$count_reports = $db->count("reports",array("reporter_id"=>$login_seller_id,"content_id"=>$proposal_id));
					if($count_reports == 0){
		?>
		<p class="<?=($lang_dir == "right" ? 'text-right':'')?>"><a class="text-danger" href="#" data-toggle="modal" data-target="#report-modal"><i class="fa fa-flag"></i> <?=$lang["report_proposal"];?></a></p>
		<?php 
				} 
			}
		}
